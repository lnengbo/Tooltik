...
/**
 * @brief GetMacIDList 得到Addressmac序列
 * @param pMacList
 * @param lstSize
 * @param pnCnt
 * @return
...
/**
 * @brief GetLicenseInfo 得到license文件
 * @param szSoftName
 * @param szLicense
 * @return
 */
...
NR_LOCAL NR_INT YearSumDay(NR_INT year,NR_INT month)
{
    NR_INT sum = 0;
    NR_INT rui[12]={31,29,31,30,31,30,31,31,30,31,30,31};
    NR_INT ping[12]={31,28,31,30,31,30,31,31,30,31,30,31};
    NR_INT ruiflag=0;
    if((year%4==0 &&year%100!=0) || year%400==0) ruiflag=1;
    for(NR_INT index=0;index<month-1;index++)
    {
        if(ruiflag==1)
            sum+=rui[index];
        else
...
NR_LOCAL NR_INT GetGALLDays(NR_INT year,NR_INT month,NR_INT day)
{
    NR_INT i = 1800,days = -24;
    while(i < year)
    {
    ...
    NR_INT days2 = YearSumDay(year,month);
    return days + days2 + day;
}
NR_LOCAL NR_STATUS CheckLicenseNewPrivate(const NR_CHAR *szSoftName,NR_INT &diffTime)
{
    NR_INT64 pMacIDList[NR_MAX_MACID_NUMBER] = {0};
    NR_UINT nCount = 0;
    ...
    for (nMacNo = 0; nMacNo < nCount && nMacNo < NR_MAX_MACID_NUMBER; ++nMacNo) {
        ...
        if (dwLicenseLen == NR_TEMPLICENSE_LEN ) { // temperary license
            ...
            if(nMon < 10)
                _mon= "0" + QString::number(nMon);
            else
                _mon = QString::number(nMon);
            if(nDay < 10)
                _day = "0" + QString::number(nDay);
            else
                _day = QString::number(nDay);
            QString regtime = QString::number(nYear) + "-" + _mon + "-" +
                    _day +" " +"00:00:00";
            QDateTime regTime = QDateTime::fromString(regtime,"yyyy-MM-dd hh:mm:ss");
            char ctime[30] = {'\0'};
            time_t curTime = time(NR_NULL);
            QSettings setting("HKEY_CURRENT_USER\\Software\\" + QString::fromLocal8Bit(szSoftName), QSettings::NativeFormat);
            QDateTime time = QDateTime::currentDateTime();
            QString currentTime = time.toString("yyyy-MM-dd hh:mm:ss");
            //第一次注册进当前时间
            if(setting.value("Templic",0).toInt() == 0)
            {
                setting.setValue("Time",currentTime);
                setting.setValue("Templic",1);
                QByteArray ba = currentTime.toLatin1();
                char *_time = ba.data();
                memcpy(ctime,_time,30);
            }
            else
            {
                //当前最新时间
                QString stime = setting.value("Time").toString();
                //qDebug() << "settings Time" << stime;
                QDateTime time2 = QDateTime::fromString(stime,"yyyy-MM-dd hh:mm:ss");
                int day = time.daysTo(time2);
                //如果最新则重新注册
                if(day < 0)
                {
                    setting.setValue("Time",currentTime);
...
NR_EXTERN LICENSE_EXPORT NR_STATUS NR_API CheckLicense(NR_IN const NR_CHAR *szSoftName,NR_OUT NR_INT &diffTime)
{
    return CheckLicenseNewPrivate(szSoftName,diffTime);
}
